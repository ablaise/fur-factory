...
function Font:draw(text, x, y, opacity)
  if text ~= "" then
    ...
    -- creating the black border and the text
...
function Font:isHover()
  return love.mouse.getX() >= self.x 
     and love.mouse.getX() <= (self.x + self:getTextWidth()) 
     and love.mouse.getY() >= self.y 
     and love.mouse.getY() <= (self.y + self:getTextHeight())
end
